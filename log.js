...
function fetchData(){
    fetch('http://localhost/studentlog.php',{
        method:'GET',
        headers:{
    ...
    document.getElementById('loginForm').addEventListener('submit',function(event){
        ...
        if(data && Array.isArray(data) && data.length>0){
            ...
            for(i = 0;i < data.length;i++){
                if(enteredMail === data[i].semail && enterPassword === data[i].spassword){
                    userFound = true;
                    var filtermail = enteredMail;
                    var filtername = data[i].studentid;
            ...
            if(userFound){
                ...
                window.location.href ='A:/mini web/student/student.html' + '?filtername=' + filtername;
            }
            else if(enteredMail == ""){
